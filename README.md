...
This repository contains a Node.js 24 CLI, a local MCP bridge, protected identity storage, encrypted-keystore import/backup, and Ethereum verification adapters. It is **not yet the public one-prompt release**. Spec #1 remains open; see [remaining acceptance work](docs/acceptance.md).
...
The trading preview supports Uniswap V3 exact-input swaps through the officially documented SwapRouter02, QuoterV2 and Robinhood WETH9 deployments. Inputs may be ERC-20 or native ETH. `trade quote` discovers the best standard fee-tier route and returns unsigned intent; `--fee` remains an advanced override. `trade authorize` requires local interactive confirmation for one exact account/trade and bounded gas; `trade execute` uses that permission. A separately activated bounded policy can authorize explicit fixed-amount or balance-percentage quick buys within token, amount, frequency, native-reserve, slippage, gas, fee and expiry limits. Use a separate state directory if the trading account differs from the Gossip Identity Wallet; the quoted account must match the wallet explicitly configured in that directory.
...
bounded autonomy before trading. Durable watchers and DCA schedules run through
a singleton foreground worker, use stable occurrence IDs, and reuse the same
transaction journal as quick buys. See
[the autonomous trading contract](docs/autonomous-trading.md).
...
Execution is tracked in epic #3,
